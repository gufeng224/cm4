...
#    parser.add_argument("--p4_change", help="Write the $Change for P4",action="store_true")
    ...
    parser.add_argument("--mkfile", help="make file name", default="############")
...
#        args.chip_id += '#'
    ...
    while len(args.mkfile) < 8 :
        args.mkfile += '#'

    while len(args.project) < 2 :
        args.project += '#'
...
#    version_file.write('#define MVXV_HEAD_VER   "' + '3'+'"\n')
...
#    version_file.write('#define MVXV_COMP_ID    "' + args.comp_id + '"\n')
    version_file.write('#define MVXV_MKFILE "' + args.project+'/'+args.mkfile + '"\n')
    if len(ext_str) > 0 :
